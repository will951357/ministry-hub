...
export function DataFilters({ onFilterChange, filterOptions, paymentMethods = [] }: DataFiltersProps) {
  const [activeFilters, setActiveFilters] = useState<FilterOption[]>([]);
  const [filterValues, setFilterValues] = useState<FilterValues>({});

  const unusedFilters = filterOptions.filter(
    filter => !activeFilters.some(af => af.id === filter.id)
  );

  const addFilter = (filterId: string) => {
    const filterToAdd = filterOptions.find(f => f.id === filterId);
    ...
    const updatedFilters = activeFilters.map(filter => {
      if (filter.id === filterId) {
        ...
        if (filterId === "date") {
          ...
          displayValue = method?.label || value;
