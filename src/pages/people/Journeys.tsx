          ...
          <div className="space-y-6 py-4">
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input id="journeyName" placeholder="e.g., Prayer Warrior, Bible Study, etc." value={newJourney.name} onChange={e => setNewJourney({
                ...newJourney,
                name: e.target.value
              })} />
              </div>
              <div className="space-y-2">
                ...
                <Textarea id="journeyDescription" placeholder="Describe the purpose and goals of this journey..." value={newJourney.description} onChange={e => setNewJourney({
                ...newJourney,
            ...
            <div className="space-y-4">
              ...
              {newJourney.steps.length === 0 && <div className="text-center py-6 border border-dashed rounded-md">
              ...
              <div className="space-y-4">
                {newJourney.steps.map((step, stepIndex) => <Card key={step.id} className="relative">
                    <CardHeader className="pb-2">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-medium text-church-accent">
                          Step {stepIndex + 1}
                        </span>
                        <Button variant="ghost" size="sm" onClick={() => removeStep(step.id)} className="h-8 w-8 p-0 text-destructive">
                    ...
                    <CardContent className="space-y-4">
                      <div className="grid grid-cols-3 gap-4">
                        <div className="col-span-2">
                          ...
                          <Input id={`step-name-${step.id}`} placeholder="e.g., Complete Daily Prayer" value={step.name} onChange={e => updateStep(step.id, 'name', e.target.value)} />
                        </div>
                        <div>
                          ...
                          <Input id={`step-points-${step.id}`} type="number" min="0" placeholder="10" value={step.points || ''} onChange={e => updateStep(step.id, 'points', parseInt(e.target.value) || 0)} />
                        </div>
                      </div>
                        ...
                        <CollapsibleTrigger asChild>
                          <Button variant="ghost" size="sm" className="w-full justify-between border border-dashed">
                            <span className="flex items-center">
                              ...
                              {step.subSteps.length > 0 ? `Sub-steps (${step.subSteps.length})` : "Add sub-steps (optional)"}
                            </span>
                          </Button>
                        </CollapsibleTrigger>
                        <CollapsibleContent className="mt-2 space-y-2">
                          {step.subSteps.map(subStep => <div key={subStep.id} className="flex items-center space-x-2">
                              <Input placeholder="e.g., Attend Sunday service" value={subStep.name} onChange={e => updateSubStep(step.id, subStep.id, e.target.value)} className="flex-1" />
                              <Button variant="ghost" size="sm" onClick={() => removeSubStep(step.id, subStep.id)} className="h-8 w-8 p-0 text-destructive">
                                <X className="h-4 w-4" />
                              </Button>
                            </div>)}
                          <Button variant="ghost" size="sm" onClick={() => addSubStep(step.id)} className="w-full justify-center border border-dashed mt-2 text-church-accent">
                            <Plus className="h-3 w-3 mr-1" />
